...
public class EdgeConvertGUI {
        ...
        new ListSelectionListener() {
          public void valueChanged(ListSelectionEvent lse) {
            ...
            if (selIndex >= 0) {
              ...
              enableControls();
              jrbDataType[currentDTField.getDataType()].setSelected(true); // select the appropriate radio button, based
                                                                           // on value of dataType
              if (jrbDataType[0].isSelected()) { // this is the Varchar radio button
                jbDTVarchar.setEnabled(true); // enable the Varchar button
                jtfDTVarchar.setText(Integer.toString(currentDTField.getVarcharValue())); // fill text field with
                                                                                          // varcharValue
              ...
              switch (i) {
                ...
                case 2: // Integer
                  try {
                    ...
                    JOptionPane.showMessageDialog(null,
                        "\"" + result + "\" is not an integer or is outside the bounds of valid integer values.");
                  }
                  break;
                case 3: // Double
                  try {
    ...
    jbDRBindRelation.addActionListener(
        new ActionListener() {
          public void actionPerformed(ActionEvent ae) {
            ...
            if (currentDRField1.getFieldBound() == relatedField) { // the selected fields are already bound to each
                                                                   // other
  ...
  protected void getOutputClasses() {
    ...
    try {
      for (int i = 0; i < resultFiles.length; i++) {
        ...
        if (resultClass.getSuperclass().getName().equals("EdgeConvertCreateDDL")) { // only interested in classes that
                                                                                    // extend EdgeConvertCreateDDL
    ...
    if (alProductNames.size() > 0 && alSubclasses.size() > 0) { // do not recreate productName and objSubClasses arrays
                                                                // if the new path is empty of valid files
  ...
  // Getter methods for Listener classes
  protected ExampleFileFilter getEffEdge() {
    return effEdge;
  }

  protected ExampleFileFilter getEffSave() {
    return effSave;
  }

  protected EdgeTable[] getTables() {
    return tables;
  }

  protected void setTables(EdgeTable[] value) {
    this.tables = value;
  }

  protected EdgeField[] getFields() {
    return fields;
  }

  protected void setFields(EdgeField[] value) {
    this.fields = value;
  }

  protected JRadioButton[] getJrbDataType() {
    return jrbDataType;
  }

  protected JTextField getJtfDTVarchar() {
    return jtfDTVarchar;
  }

  protected JButton getJbDTVarchar() {
    return jbDTVarchar;
  }

  protected JTextField getJtfDTDefaultValue() {
    return jtfDTDefaultValue;
  }

  protected EdgeField getCurrentDTField() {
    return currentDTField;
  }

  protected File getParseFile() {
    return parseFile;
  }

  protected void setParseFile(File value) {
  ...
  protected void setSaveFile(File value) {
    this.saveFile = value;
  }

  protected EdgeConvertFileParser getEcfp() {
    return ecfp;
  }

  protected void setEcfp(EdgeConvertFileParser value) {
    this.ecfp = value;
  }
  ...
  protected void setDataSaved(boolean value) {
  ...
  protected JMenuItem getJmiDTOpenEdge() {
    return jmiDTOpenEdge;
  }

  protected JMenuItem getJmiDTOpenSave() {
    return jmiDTOpenSave;
  }

  protected JMenuItem getJmiDROpenSave() {
    return jmiDROpenSave;
  }

  protected JMenuItem getJmiDTExit() {
    return jmiDTExit;
  }

  protected JMenuItem getJmiDRExit() {
    return jmiDRExit;
  }

  protected JFileChooser getJfcEdge() {
    return jfcEdge;
  }

  protected JFrame getJfDT() {
    return jfDT;
  }

  protected JFrame getJfDR() {
    return jfDR;
  }

  protected JMenuItem getJmiDTSave() {
    return jmiDTSave;
  }

  protected JMenuItem getJmiDTSaveAs() {
    return jmiDTSaveAs;
  }

  protected JMenuItem getJmiDRSave() {
    return jmiDRSave;
  }

  protected JMenuItem getJmiDRSaveAs() {
    return jmiDRSaveAs;
  }

  protected JMenuItem getJmiDTOptionsOutputLocation() {
    return jmiDTOptionsOutputLocation;
  }

  protected JMenuItem getJmiDROptionsOutputLocation() {
    return jmiDROptionsOutputLocation;
  }

  protected JMenuItem getJmiDTOptionsShowProducts() {
    return jmiDTOptionsShowProducts;
  }

  protected JMenuItem getJmiDROptionsShowProducts() {
    return jmiDROptionsShowProducts;
  }

  protected JMenuItem getJmiDTHelpAbout() {
    return jmiDTHelpAbout;
  }

  protected JMenuItem getJmiDRHelpAbout() {
    return jmiDRHelpAbout;
  }

  protected JButton getJbDTDefineRelations() {
    return jbDTDefineRelations;
  }

  protected JButton getJbDTCreateDDL() {
    return jbDTCreateDDL;
  }

  protected JButton getJbDRCreateDDL() {
    return jbDRCreateDDL;
  }
} // EdgeConvertGUI
