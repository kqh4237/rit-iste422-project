...
public class CreateDDLMySQL extends EdgeConvertCreateDDL {
  ...
  public void createDDL() {
    ...
    for (int boundCount = 0; boundCount <= maxBound; boundCount++) { // process tables in order from least dependent
                                                                     // (least number of bound tables) to most dependent
        ...
        if (numBoundTables[tableCount] == boundCount) { //
          ...
          for (int nativeFieldCount = 0; nativeFieldCount < nativeFields.length; nativeFieldCount++) { // print out the
            ...
            if (!currentField.getDefaultValue().equals("")) {
              ...
              } else if (currentField.getDataType() == 0) { // varchar data type
            ...
            // BUGFIX Checks to make sure it is not last field before adding ","
